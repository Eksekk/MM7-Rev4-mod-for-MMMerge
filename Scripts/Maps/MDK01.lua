...
evt.map[376] = function()
	if not evt.Cmp("QBits", 704) then         -- Turn on map in mdkXX(Dwarven Barrow)
		evt.ForPlayer("All")
		if evt.Cmp("Inventory", 1458) then         -- "Barrow Key"
			evt.SetDoorState{Id = 25, State = 0}
			evt.SetDoorState{Id = 26, State = 0}
			evt.Add("QBits", 704)         -- Turn on map in mdkXX(Dwarven Barrow)
			evt.Subtract("Inventory", 1458)         -- "Barrow Key"
		end
	end
end
