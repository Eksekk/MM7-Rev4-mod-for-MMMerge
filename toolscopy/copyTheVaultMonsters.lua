...
function persistUpdatedRev4()
	...
	local file = assert(io.open("Editor.State.Monsters the vault rev4.bin", "wb"), "Cannot open file for writing")
	file:write(str)
	file:close()
end
...
function unpersistFromRev4()
	local file = assert(io.open("Editor.State.Monsters the vault rev4.bin", "rb"), "Cannot open file for reading")
...
------ COPY CORRECT POSITIONS -----
-- intended to be run from rev4 after just entering the vault
...
print(dump(positions, 2, true))

-- intended to be run from editor in Merge
-- but first define previous dump as global "positions"

for v, k in pairs(Editor.State.Monsters) do
	if not positions[k] then print ("Missing position") break end
	v.X = positions[k][1]
	v.Y = positions[k][2]
	v.Z = positions[k][3]
end
