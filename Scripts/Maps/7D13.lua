...
evt.map[376] = function()
	if evt.Cmp("QBits", 539) then         -- "Find the lost meditation spot in the Dwarven Barrows."
...
evt.map[377] = function()
	...
	if not evt.Cmp("QBits", 577) then         -- Barrow downs.   Returned the bones of the Dwarf King.  Arch Druid promo quest.
		if evt.Cmp("QBits", 566) then         -- "Retrieve the bones of the Dwarf King from the tunnels between Stone City and Nighon and place them in their proper resting place in the Barrow Downs, then return to Anthony Green in the Tularean Forest."
			if evt.Cmp("Inventory", 1428) then         -- "Zokarr IV's Skull"
				evt.Subtract("Inventory", 1428)         -- "Zokarr IV's Skull"
				evt.Set("QBits", 577)         -- Barrow downs.   Returned the bones of the Dwarf King.  Arch Druid promo quest.
				evt.ForPlayer("All")
				evt.Add("QBits", 757)         -- "Congratulations"
				evt.Subtract("QBits", 757)         -- "Congratulations"
				evt.Subtract("QBits", 740)         -- Dwarf Bones - I lost it
			end
		end
	end
end
