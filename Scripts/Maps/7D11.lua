...
evt.map[176] = function()
	if not evt.Cmp("MapVar98", 1) then
		if not evt.Cmp("QBits", 560) then         -- "Retrieve the lich jars from the Proving Grounds in Celeste and bring them back to Halfgild Wynac in the Pit."
			evt.OpenChest(1)
			return
		end
	end
	evt.OpenChest(0)
	evt.Set("MapVar98", 1)
	evt.Add("QBits", 660)         -- Got Lich Jar from Walls of Mist
end
...
evt.map[195] = function()
	evt.OpenChest(0)
	evt.Subtract("QBits", 741)         -- Lich Jar (Empty) - I lost it
end
...
evt.map[376] = function()
	...
	if evt.Cmp("Inventory", 1454) then         -- "West Pillar Key"
		evt.Subtract("Inventory", 1454)         -- "West Pillar Key"
...
evt.map[377] = function()
	...
	if evt.Cmp("Inventory", 1455) then         -- "Central Pillar Key"
		evt.Subtract("Inventory", 1455)         -- "Central Pillar Key"
...
evt.map[378] = function()
	...
	if evt.Cmp("Inventory", 1456) then         -- "East Pillar Key"
		evt.Subtract("Inventory", 1456)         -- "East Pillar Key"
...
evt.map[502] = function()
	if not evt.CheckMonstersKilled{CheckType = 1, Id = 52, Count = 1} then
		evt.ForPlayer("All")
		evt.Set("QBits", 614)         -- Completed Proving Grounds without killing a single creature
	end
	evt.MoveToMap{X = 1728, Y = 3648, Z = 97, Direction = 1024, LookAngle = 0, SpeedZ = 0, HouseId = 0, Icon = 8, Name = "7D25.blv"}
end
