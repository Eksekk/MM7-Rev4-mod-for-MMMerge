...
evt.map[1] = function()  -- function events.LoadMap()
	if evt.Cmp("QBits", 704) then         -- Turn on map in mdkXX(Dwarven Barrow)
...
evt.map[177] = function()
	if evt.Cmp("Awards", 30) then         -- "Returned Withern's Lantern"
		evt.OpenChest(0)
	else
		evt.OpenChest(2)
	end
...
evt.map[501] = function()
	if evt.Cmp("MapVar1", 2) then
		...
		evt.MoveToMap{X = 2074, Y = -281, Z = -256, Direction = 384, LookAngle = 0, SpeedZ = 0, HouseId = 0, Icon = 0, Name = "MDK04.blv"}
	end
end
...
evt.map[502] = function()
	if evt.Cmp("MapVar0", 2) then
		evt.MoveToMap{X = 1521, Y = -1256, Z = 0, Direction = 1792, LookAngle = 0, SpeedZ = 0, HouseId = 0, Icon = 0, Name = "MDT05.blv"}
	else
