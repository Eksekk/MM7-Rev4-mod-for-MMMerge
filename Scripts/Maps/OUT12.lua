...
evt.map[1] = function()  -- function events.LoadMap()
	if evt.Cmp("QBits", 775) then         -- Area 12 Archibald only once
		return
	end
	if not evt.Cmp("QBits", 616) then         -- "Go to Colony Zod in the Land of the Giants and slay Xenofex then return to Resurectra in Castle Lambent in Celeste."
		if evt.Cmp("QBits", 635) then         -- "Go to Colony Zod in the Land of the Giants and slay Xenofex then return to Kastore in the Pit."
...
Please rescue him!  Not even I can bear to think of my brother in those conditions!  To help you along, I offer this weapon.  It was...found by my loyal servant sergeant Piridak, amongst my advisor's personal belongings.  I hope it helps." ]]
			evt.Add("QBits", 775)         -- Area 12 Archibald only once
			evt.SpeakNPC(462)         -- "Archibald Ironfist"
			evt.Add("Inventory", 866)         -- "Blaster"
			return
		end
	end
	evt.Add("QBits", 775)         -- Area 12 Archibald only once
...
evt.HouseDoor(52, 1150)  -- "Lasiter's Home"
...
evt.map[452] = function()
	evt.Set("QBits", 758)         -- Visited The Land of the giants
...
evt.map[453] = function()
	if not evt.Cmp("QBits", 686) then         -- Visited Obelisk in Area 12
		evt.StatusText(51)         -- "veoseo_l"
		evt.Add("AutonotesBits", 319)         -- "Obelisk message #11: veoseo_l"
		evt.ForPlayer("All")
		evt.Add("QBits", 686)         -- Visited Obelisk in Area 12
	end
end
