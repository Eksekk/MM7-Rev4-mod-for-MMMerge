...
evt.map[1] = function()  -- function events.LoadMap()
	if evt.Cmp{"QBits", Value = 705} then         -- Turn on map in mdrXX(Dwarven Barrow)
...
evt.map[376] = function()
	if not evt.Cmp{"QBits", Value = 705} then         -- Turn on map in mdrXX(Dwarven Barrow)
		evt.ForPlayer(-- ERROR: Const not found
"All")
		if evt.Cmp{"Inventory", Value = 1459} then         -- "Barrow Key"
			evt.SetDoorState{Id = 25, State = 0}
			evt.SetDoorState{Id = 26, State = 0}
			evt.Add{"QBits", Value = 705}         -- Turn on map in mdrXX(Dwarven Barrow)
			evt.Subtract{"Inventory", Value = 1459}         -- "Barrow Key"
		end
	end
end
...
evt.map[502] = function()
	if evt.Cmp{"MapVar0", Value = 2} then
		evt.MoveToMap{X = 10, Y = -985, Z = 1, Direction = 256, LookAngle = 0, SpeedZ = 0, HouseId = 0, Icon = 0, Name = "MDK05.blv"}
	else
		evt.MoveToMap{X = -27, Y = 591, Z = 1, Direction = 768, LookAngle = 0, SpeedZ = 0, HouseId = 0, Icon = 0, Name = "MDK01.blv"}
	end
end
