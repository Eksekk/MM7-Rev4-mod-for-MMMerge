...
evt.map[376] = function()
	if not evt.Cmp("QBits", 705) then         -- Turn on map in mdrXX(Dwarven Barrow)
		evt.ForPlayer("All")
		if evt.Cmp("Inventory", 1459) then         -- "Barrow Key"
			evt.SetDoorState{Id = 25, State = 0}
			evt.SetDoorState{Id = 26, State = 0}
			evt.Add("QBits", 705)         -- Turn on map in mdrXX(Dwarven Barrow)
			evt.Subtract("Inventory", 1459)         -- "Barrow Key"
		end
	end
end
