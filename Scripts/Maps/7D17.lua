...
evt.map[176] = function()
	if not evt.Cmp("MapVar2", 1) then
		if evt.Cmp("QBits", 1607) then         -- "Promoted to Priest"
			goto _8
		end
		if evt.Cmp("QBits", 1608) then         -- "Promoted to Honorary Priest"
			goto _8
		end
		if evt.Cmp("QBits", 730) then         -- Map to Evenmorn - I lost it
			goto _8
		end
	end
	evt.OpenChest(1)
	evt.Add("QBits", 730)         -- Map to Evenmorn - I lost it
	evt.Set("MapVar2", 1)
	do return end
::_8::
	evt.OpenChest(0)
