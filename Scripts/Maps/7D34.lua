...
evt.map[1] = function()  -- function events.LoadMap()
	if evt.Cmp("QBits", 570) then         -- Destroyed critter generator in dungeon.  Warrior Mage promo quest.
		evt.Set("MapVar0", 1)
	end
...
::_9::
	...
	do return end
::_8::
	evt.Add("Inventory", 1491)         -- "Kergar-laced ore"
	goto _9
end
...
evt.map[376] = function()
	evt.ForPlayer("All")
	if evt.Cmp("Inventory", 1431) then         -- "Elixir"
		evt.SetSprite{SpriteId = 1, Visible = 0, Name = "0"}
...
evt.map[383] = function()
	if evt.Cmp("QBits", 543) then         -- "Sabotage the lift in the Red Dwarf Mines in the Bracada Desert then return to Zedd True Shot on Emerald Island."
		if not evt.Cmp("QBits", 570) then         -- Destroyed critter generator in dungeon.  Warrior Mage promo quest.
			evt.ForPlayer("All")
			if evt.Cmp("Inventory", 1451) then         -- "Worn Belt"
				evt.Set("QBits", 570)         -- Destroyed critter generator in dungeon.  Warrior Mage promo quest.
				evt.Subtract("Inventory", 1451)         -- "Worn Belt"
				evt.Subtract("QBits", 728)         -- Worn Belt - I lost it
				evt.Set("Counter4", 0)
