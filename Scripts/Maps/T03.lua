...
evt.map[376] = function()
	if not evt.Cmp{"QBits", Value = 575} then         -- Defaced the Altar of Good.  Priest of Dark promo quest.
		if evt.Cmp{"QBits", Value = 556} then         -- "Deface the Altar of Good in the Temple of the Sun on Evenmorn Isle then return to Daedalus Falk in the Deyja Moors."
			evt.SetTexture{Facet = 20, Name = "Cfb1"}
			evt.Set{"QBits", Value = 575}         -- Defaced the Altar of Good.  Priest of Dark promo quest.
			evt.ForPlayer(-- ERROR: Const not found
"All")
			evt.Add{"QBits", Value = 757}         -- "Congratulations"
			evt.Subtract{"QBits", Value = 757}         -- "Congratulations"
			evt.StatusText{Str = 15}         -- "You have Desecrated the altar"
		end
	end
end
