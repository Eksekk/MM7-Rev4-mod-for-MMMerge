...
local function CanApplySlowMassDistort(d)
	...
	local Res = _G.getEffectiveResistance(Mon, PL, const.Damage.Earth)
...
mem.hook(0x42c42a, function(d)
	...
	local res = _G.getEffectiveResistance(mon, Player, const.Damage.Dark)

	if res == const.MonsterImmune or Game.IsMonsterOfKind(mon.Id, const.MonsterKind.Undead) == 0 then
		d.eax = 0
		return
	end

	if res > Skill*Mas then
		d.eax = 0
		return
	end
