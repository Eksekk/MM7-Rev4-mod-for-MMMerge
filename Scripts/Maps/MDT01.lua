...
evt.map[1] = function()  -- function events.LoadMap()
	if evt.Cmp{"QBits", Value = 703} then         -- Turn on map in mdtXX(Dwarven Barrow)
...
evt.hint[21] = evt.str[12]  -- "Torch"
Game.MapEvtLines:RemoveEvent(21)
evt.map[21] = function()
	evt.MoveToMap{X = 412, Y = 298, Z = -15, Direction = 1024, LookAngle = 0, SpeedZ = 0, HouseId = 0, Icon = 0, Name = "0"}
end
...
evt.map[376] = function()
	if not evt.Cmp{"QBits", Value = 703} then         -- Turn on map in mdtXX(Dwarven Barrow)
		evt.ForPlayer(-- ERROR: Const not found
"All")
		if evt.Cmp{"Inventory", Value = 1457} then         -- "Barrow Key"
			evt.SetDoorState{Id = 25, State = 0}
			evt.SetDoorState{Id = 26, State = 0}
			evt.Add{"QBits", Value = 703}         -- Turn on map in mdtXX(Dwarven Barrow)
			evt.Subtract{"Inventory", Value = 1457}         -- "Barrow Key"
		end
	end
end
...
evt.map[502] = function()
	if evt.Cmp{"MapVar0", Value = 2} then
		evt.MoveToMap{X = -27, Y = 591, Z = 1, Direction = 512, LookAngle = 0, SpeedZ = 0, HouseId = 0, Icon = 0, Name = "MDK01.blv"}
	else
		evt.MoveToMap{X = 2036, Y = 3142, Z = 0, Direction = 260, LookAngle = 0, SpeedZ = 0, HouseId = 0, Icon = 0, Name = "MDR03.blv"}
	end
end
