...
"All")
	if not evt.Cmp{"QBits", Value = 840} then         -- Titan Sprite C
		evt.Add{"AirResistance", Value = 10}
		evt.Set{"QBits", Value = 840}         -- Titan Sprite C
...
evt.map[11] = function()
	evt.ForPlayer(-- ERROR: Const not found
"All")
	if not evt.Cmp{"QBits", Value = 839} then         -- Titan Sprite B
		evt.Add{"AirResistance", Value = 10}
		evt.Set{"QBits", Value = 839}         -- Titan Sprite B
...
evt.map[12] = function()
	evt.ForPlayer(-- ERROR: Const not found
"All")
	if not evt.Cmp{"QBits", Value = 841} then         -- Titan Sprite D
		evt.Add{"MindResistance", Value = 20}
		evt.Add{"BodyResistance", Value = 20}
		evt.Add{53, Value = 20}
		evt.Set{"QBits", Value = 841}         -- Titan Sprite D
...
evt.map[16] = function()  -- function events.LoadMap()
	evt.ForPlayer(-- ERROR: Const not found
"All")
	if evt.Cmp{"QBits", Value = 839} then         -- Titan Sprite B
		evt.SetSprite{SpriteId = 11, Visible = 1, Name = "torch01"}
	end
	if evt.Cmp{"QBits", Value = 840} then         -- Titan Sprite C
		evt.SetSprite{SpriteId = 12, Visible = 1, Name = "torch01"}
	end
	if evt.Cmp{"QBits", Value = 841} then         -- Titan Sprite D
		evt.SetSprite{SpriteId = 13, Visible = 1, Name = "torch01"}
	end
end
...
evt.map[176] = function()
	if not evt.Cmp{"QBits", Value = 2098} then         -- "Promoted to Master Archer"
		if not evt.Cmp{"QBits", Value = 2099} then         -- "Promoted to Honorary Master Archer"
			if not evt.Cmp{"QBits", Value = 2100} then         -- "Promoted to Sniper"
				if not evt.Cmp{"QBits", Value = 2101} then         -- "Promoted to Honorary Sniper"
					if evt.Cmp{"QBits", Value = 542} then         -- "Retrieve the Perfect Bow from the Titans' Stronghold in Avlee and return it to Lawrence Mark in Harmondale."
						goto _8
					end
					if evt.Cmp{"QBits", Value = 544} then         -- "Retrieve the Perfect Bow from the Titans' Stronghold in Avlee and return it to Steagal Snick in Avlee."
						goto _8
					end
				end
			end
	...
	do return end
::_8::
	evt.Set{"QBits", Value = 675}         -- Got perfect bow out of chest
	evt.OpenChest{Id = 1}
end
