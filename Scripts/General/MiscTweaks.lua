	...
	function events.RegenTick(Player)
		if difficulty == const.Difficulty.Hard and Merge.ModSettings.Rev4ForMergeNerfMeditationSPRegen == 1 then return end
		local Cond = Player:GetMainCondition()
		if Cond == 18 or Cond == 17 or Cond < 14 then
			...
			if RegM > 0 then
				...
				local Add
				if difficulty == const.Difficulty.Medium and Merge.ModSettings.Rev4ForMergeNerfMeditationSPRegen == 1 then
					Add = RegM >= 3 and (RegM - 2 + math.floor(RegS/10)) or 0
				elseif difficulty == const.Difficulty.Easy or Merge.ModSettings.Rev4ForMergeNerfMeditationSPRegen ~= 1 then
					Add = RegM + math.floor(RegS/10)
				end
