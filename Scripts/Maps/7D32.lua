...
evt.map[4] = function()
	if evt.Cmp{"QBits", Value = 547} then         -- "Raid the Elven Treasury at Castle Navan in the Tularean Forest and return to Frederick Org in Erathia."
...
"All")
		evt.Set{"QBits", Value = 572}         -- Robbed Elven treasury.  Black Knight promo quest.
	end
end
...
evt.map[416] = function()
	if evt.Cmp{"QBits", Value = 610} then         -- Built Castle to Level 2 (rescued dwarf guy)
		if not evt.Cmp{"MapVar4", Value = 2} then
			evt.EnterHouse{Id = 218}         -- "Throne Room"
			return
		end
	end
...
evt.map[451] = function()
	if not evt.Cmp{"Invisible", Value = 0} then
		if not evt.Cmp{"MapVar4", Value = 1} then
			evt.SpeakNPC{NPC = 617}         -- "Castle Guard"
