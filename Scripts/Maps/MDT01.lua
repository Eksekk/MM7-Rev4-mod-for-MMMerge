...
evt.map[1] = function()  -- function events.LoadMap()
	if evt.Cmp("QBits", 703) then         -- Turn on map in mdtXX(Dwarven Barrow)
...
evt.map[376] = function()
	if not evt.Cmp("QBits", 703) then         -- Turn on map in mdtXX(Dwarven Barrow)
		evt.ForPlayer("All")
		if evt.Cmp("Inventory", 1457) then         -- "Barrow Key"
			evt.SetDoorState{Id = 25, State = 0}
			evt.SetDoorState{Id = 26, State = 0}
			evt.Add("QBits", 703)         -- Turn on map in mdtXX(Dwarven Barrow)
			evt.Subtract("Inventory", 1457)         -- "Barrow Key"
		end
	end
end
