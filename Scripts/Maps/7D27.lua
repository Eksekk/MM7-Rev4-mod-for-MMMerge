...
evt.map[2] = function()  -- function events.LeaveMap()
	if not evt.Cmp("QBits", 617) then         -- Slayed Xenofex
		if evt.CheckMonstersKilled{CheckType = 3, Id = 0, Count = 0} then
			evt.Set("QBits", 617)         -- Slayed Xenofex
...
evt.map[21] = function()
	if evt.Cmp("Inventory", 1089) then         -- "_potion/reagent287"
...
evt.map[377] = function()
	if evt.Cmp("Inventory", 1463) then         -- "Colony Zod Key"
		evt.Subtract("Inventory", 1463)         -- "Colony Zod Key"
